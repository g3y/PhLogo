...
module.exports = async (firstWord, secondWord='') => {
    ...
    if(!firstWord || firstWord.length === 0) throw new Error('Marque au moins quelque chose dans test.js.');

    if(secondWord === '') {
        var middle = Math.ceil(firstWord.length / 2);
        secondWord = firstWord.slice(middle);
        firstWord = firstWord.slice(0, middle);
        console.log(`Premier mot : ${firstWord}\nSecond mot : ${secondWord}`);
        console.log('Millieu : ' + middle)
    }
